"""
脚本3: VLM + YOLOE (无图像增强)
VLM生成目标框 → 直接作为YOLOE视觉提示 → 实时检测显示mask+框
每10s统计一次检测成功次数
"""
import cv2
import numpy as np
...
from VLM_demo import encode_image, smart_resize, resize_bbox_to_original, show_box_cv2, show_mask_cv2
...
def vlm_get_bboxes(image_path, instruction):
    ...
    Decompose composite objects into visible parts (e.g., "cup body", "cup handle").
    Do not decompose simple objects (e.g., "block", "mouse", "pen").
...
def main():
    ...
    image_path = "detection_scripts/tmp_vlm_yoloe.jpg"
    ...
    # 设置YOLOE视觉提示 (无增强)
    t0 = time.time()
    visuals, classes, label2id, cls_label = process_visual_prompt(bbox_entities)
    ...
    while True:
        ...
        cv2.imshow("VLM+YOLOE (No Aug)", cv2.cvtColor(frame_draw, cv2.COLOR_RGB2BGR))
        key = cv2.waitKey(1) & 0xFF
        if key == ord('q'):
            break
        elif key == ord('s'):
            cv2.imwrite("detection_scripts/vlm_yoloe_result.jpg",
                        cv2.cvtColor(frame_draw, cv2.COLOR_RGB2BGR))
            print("已保存结果到 detection_scripts/vlm_yoloe_result.jpg")
